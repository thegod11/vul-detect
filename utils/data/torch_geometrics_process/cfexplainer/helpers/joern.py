...
def get_node_edges(filepath: str, verbose=0, edge_type="all"):
    ...
    nodes.code = nodes.apply(lambda x: x.code if x.code != "" else x["_label"], axis=1)  ## 存在name与code都为空的结点比如block
    ...
    # remove solid nodes
    valid_ids = set(edges["innode"]).union(set(edges["outnode"]))
...
def full_run_joern(filepath: str, verbose=0):
    ...
    try:
        ...
        # 后续在 vulGraphDataset.py 中的 feature_extract 函数中已经调用get_node_edges函数
        # nodes, edges = get_node_edges(filepath)
