...
def bigvul(minimal=True, sample=False, return_raw=False, splits="default"):
    ...
    # Remove comments
    df["func_before"] = utils.dfmp(df, remove_comments, "func_before", cs=500, workers=64)
    df["func_after"] = utils.dfmp(df, remove_comments, "func_after", cs=500, workers=64)
    ...
    utils.dfmp(df, git._c2dhelper, columns=cols, ordr=False, cs=300, workers=64)

    # Assign info and save
    df["info"] = utils.dfmp(df, git.allfunc, cs=500, workers=64)
